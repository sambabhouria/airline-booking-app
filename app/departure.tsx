...
const DepartureScreen = () => {
  ...
  const [flightOfferData, setFlightOfferData] = useState<
    Pick<FlightOfferDataProps, 'originLocationCode'>
  >({
    originLocationCode: '',
  })
  const [previousSelectedDeparture, setPreviousSelectedDeparture] = useState([])

  let previousSelectedCities: any
  ...
  const handleSelectAutocompete = async (item: any) => {
    previousSelectedCities = [...previousSelectedDeparture]
      ...
      <View className="w-full h-full">
        ...
        <View className="w-full py-4 px-4 relative">
          ...
          <View className="px-4 w-full">
            ...
            {previousSelectedDeparture.map((city: any, index) => (
              <Pressable
                key={index}
                onPress={() => {
                  setFlightOfferData({
                    ...flightOfferData,
                    originLocationCode: city.code,
                  })
